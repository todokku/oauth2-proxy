...
// WithHeader replaces the request header map with the given header map.
...
// Into performs the request and attempts to unmarshal the response into the
...
func (r *builder) Into(into interface{}) error {
	resp, err := r.Do()
	if err != nil {
		return err
	}

	return Into(resp, into)
}

// JSON performs the request and attempts to unmarshal the response into a
// simplejson.Json. The response body is assume to be JSON.
// The response must have a 200 status otherwise an error will be returned.
func (r *builder) JSON() (*simplejson.Json, error) {
	resp, err := r.Do()
	if err != nil {
		return nil, err
	}
...
// Into attempts to unmarshal the response into the the given interface.
// The response body is assumed to be JSON.
// The response must have a 200 status otherwise an error will be returned.
func Into(resp *http.Response, into interface{}) error {
	body, err := getResponseBody(resp)
	if err != nil {
		return err
	}
